...
window.JCCatalogItem.prototype.QuantitySet = function(index)
{
	if (0 === this.errorCode)
	{
		...
		var $btnSubscribe = $(this.obProduct).find('button.inform-when-available');
...
window.JCCatalogItem.prototype.ChangeInfo = function()
{
	...
	if (-1 < index)
	{
		...
		var $btnPriceDrop = $(this.obProduct).find('button.inform-when-price-drops');
		...
		var $btnPriceCry = $(this.obProduct).find('button.cry-for-price');
		...
		var $btnSubscribe = $(this.obProduct).find('button.inform-when-available').add( $(this.obAvailInfo).find('.when-out-of-stock .info-tag') );
		...
		if (this.showPercent) {
			var $stickers = $(this.obProduct).find('div.stickers');
			var $discount = $stickers.find('div.discount-w-number');
			if ($discount.length) {
				...
				if (parseFloat(this.offers[index].PRICE.DISCOUNT_DIFF) > 0) {
					$stickers.append('<div class="sticker discount-w-number"><span class="text">' + this.offers[index].PRICE.DISCOUNT_DIFF_PERCENT + '%</span></div>');
				}
			}
		}
