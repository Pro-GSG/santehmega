function UmCoolSlider(cs, opt){
	var t = this,
		...
		period = period || 10000, // time in ms
	...
	t.update = function(){
		if ( csSlides.length < 2 ){
			...
			csCtrlItems.not(':last-child').find('.bar-wrap').css('width', csBarMaxW);
			csCtrlItems.eq(csCtrlItems.length-1).find('.bar-wrap').css('width', track.outerWidth());
			t.updateHeights();
			t.start();
		}
	}
