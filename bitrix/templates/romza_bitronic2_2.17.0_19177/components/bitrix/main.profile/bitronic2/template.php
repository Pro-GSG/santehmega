...
<form method="post" name="form1" action="<?=$arResult["FORM_TARGET"]?>" enctype="multipart/form-data" class="form_account-settings">
	...
	<div class="expandable allow-multiple-expanded expanded">
		...
		<div class="expand-content">
			<?foreach($arFields['MAIN'] as $code => $arFiled):
				switch($code):
					case 'LOGIN':?>
						<input type="hidden" name="<?=$code?>" id="<?=$code?>" value="<?=$arResult["arUser"][$code]?>" />
					<?break;
					case 'NEW_PASSWORD':
					...
					case 'NEW_PASSWORD_CONFIRM':
						...
						<label class="line-wrap" for="<?=$code?>">
							<span class="label-text text"><?=GetMessage($code)?><?if($arFiled['REQUIRED']):?><span class="required-asterisk">*</span><?endif?>:</span>
							<span class="content">
								<input 
									type="password" 
									name="<?=$code?>" 
									...
									<div class="clearfix">
										...
										<div class="item-actions">
											...
											<label class="checkbox-styled">
												<input type="checkbox" id="PERSONAL_PHOTO_del" name="PERSONAL_PHOTO_del" value="Y">
	...
	<button class="btn-main" name="save" value="Y"><span class="text"><?=GetMessage('SAVE')?></span></button>

</form>
<?
if($arResult["SOCSERV_ENABLED"])
{
	$APPLICATION->IncludeComponent("bitrix:socserv.auth.split", "bitronic2", array(
			"SHOW_PROFILES" => "Y",
			"ALLOW_DELETE" => "Y"
		),
		false
