...
<script type="text/javascript">
	...
	function init_<?echo $arParams['MAP_ID']?>() {
		...
		if (window.<?echo $arParams['ONMAPREADY']?>) {
			...
			?>
			<?echo $arParams['ONMAPREADY_PROPERTY']?>
			= map;
	...
	?>
	<?=CUtil::JSEscape($arParams['WAIT_FOR_EVENT'])?>
	= BXMapLoader_<?=$arParams['MAP_ID']?>;
	<?
...
<div id="BX_YMAP_<? echo $arParams['MAP_ID'] ?>" class="bx-yandex-map"
	 style="height: <? echo $arParams['MAP_HEIGHT']; ?>; width: <? echo $arParams['MAP_WIDTH'] ?>;"><? echo GetMessage('MYS_LOADING' . ($arParams['WAIT_FOR_EVENT'] ? '_WAIT' : '')); ?></div>