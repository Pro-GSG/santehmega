...
<div id="<?=$arResult["ID"]?>_div" class="jwPlayer"><?= GetMessage('PLAYER_LOADING')?></div>
<script>
window.bxPlayerOnload<?=$arResult["ID"]?> = function(config)
{
	...
	var $div = $('#<?=$arResult["ID"]?>_div').parent();
	if ($div.hasClass('video')) {
		$div = $div.end();
		mobileHack = true;
	}
	$div.removeAttr('style').addClass("jwPlayer");

	if (mobileHack) {
		jwplayer("<?=$arResult["ID"]?>_div").onReady(function(){
			var obFullSize = {'width':'100%', 'height':'100%'};
			$("#<?=$arResult["ID"]?>_div_jwplayer_display_iconBackground").css({'margin':'-25px 0 0 -25px', 'top':'50%', 'left':'50%'});
			$("#<?=$arResult["ID"]?>_div_jwplayer_display").css(obFullSize);
			$("#<?=$arResult["ID"]?>_div_displayarea").css(obFullSize);
			$("#<?=$arResult["ID"]?>_div_video").css(obFullSize);
			obFullSize['position'] = 'absolute';
			$("#<?=$arResult["ID"]?>_div").css(obFullSize);
		});
	}
