...
<div class="edost_main edost_template_div">
	<h3 class="edost_location_head">Местоположение доставки</h3>
<?
...
<?
  ...
  if (isset($arResult['edost']['warning']) && empty($arResult['edost']['locations_installed'])) echo $arResult['edost']['warning'].'<br>';
?>
...
<style>
	div.edost_main { font-family: arial; line-height: normal; margin: 35px 0 40px 0; }
...
<div class="bx_section">
	...
	if(!empty($arResult["DELIVERY"]))
	{
		...
		foreach ($arResult["DELIVERY"] as $delivery_id => $arDelivery)
		{
			...
			else if ($draw_price && isset($arDelivery['PRICE_FORMATED'])) $price = $arDelivery['PRICE_FORMATED'];
			...
			if (count($arDelivery["STORE"]) > 0) $clickHandler = "onClick = \"fShowStore('".$arDelivery["ID"]."','".$arParams["SHOW_STORES_IMAGES"]."','".$width."','".SITE_ID."')\";";
				...
				<div class="bx_element">
					...
					<label for="ID_DELIVERY_ID_<?=$arDelivery["ID"]?>">
						...
						<div class="bx_description">
							...
							<strong <?=$clickHandler?> style="<?=$top?>">
								<div class="name">
									<strong class="bitrix_title"><?= htmlspecialcharsbx($arDelivery["NAME"])?><?=(!empty($arDelivery["PERIOD_TEXT"]) && $draw_price ? '<span style="font-weight: normal;">, '.$arDelivery["PERIOD_TEXT"].'</span>' : '')?><?=($price !== false ? ' - '.$price : '')?></strong>
								</div>
							</strong>

							<? if ($profile === false && $price === false) { ?>
							<span class="bx_result_price">
								...
								else if (isset($arDelivery["CALCULATE_ERRORS"])) {
					...
					<div class="bitrix_description">
			            <?=(isset($arDelivery['office']) ? $arDelivery['office'].'<br>' : '')?>

						<?
						if (strlen($arDelivery["DESCRIPTION"]) > 0) echo $arDelivery["DESCRIPTION"]."<br />";
...
<div class="edost_main edost_template_div"<?=($address_hide ? ' style="display: none;"' : '')?>>
	<h3 id="edost_location_address_head" class="edost_location_head"<?=($address_hide ? ' style="display: none;"' : '')?>>Адрес доставки</h3>
