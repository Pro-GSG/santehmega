...
?>
<div class="viewed-products detail-slider<? if($arParams['STICKER'] == 'WITH_GIFT'): ?> gift-slider<? endif ?><?= $bHoverMode ? ' __expanded' : '' ?>">
		...
		<div class="scroll-slider frame">
			<div class="slider-content slidee catalog"><?
				...
				?><div class="slider-item<?=(!$arItem['CAN_BUY'] && !$arItem['ON_REQUEST'] ? ' out-of-stock' : '')?> catalog-item" id="<?=$arItemIDs['ID']?>">
					<a class="photo" href="<?=$arItem['DETAIL_PAGE_URL']?>">
						<span data-picture data-alt="<?=$arItem['PICTURE_PRINT']['ALT']?>">
							<span data-src="<?=$arItem['PICTURE_PRINT']['SRC']?>"></span>
					...
					<?else:?>
						...
						<span class="price" id="<?=$arItemIDs['PRICE']?>">
							<?=($arItem['bOffers']) ? GetMessage('BITRONIC2_BIGDATA_FROM') : ''?>
