...
<div class="cool-slider" id="cool-slider" data-coolslider-enabled="true"
		...
		<div class="slides">
			<?foreach($arResult['ITEMS'] as $arItem):
				...
				<div class="slide <?=($arItem['bFirst']) ? 'active' : ''?>" id="<?=$arItemIDs['ID']?>">
					<a href="<?=$arItem['DETAIL_PAGE_URL']?>" class="cool-img">
						<span data-picture data-alt="<?=$arItem['PICTURE_PRINT_BIG']['ALT']?>">
							<span data-src="<?=$arItem['PICTURE_PRINT_BIG']['SRC']?>"></span>
							<span data-src="" data-media="(max-width: 991px)"></span>
							
							<!-- Fallback content for non-JS browsers. Same img src as the initial, unqualified source element. -->
							<noscript>
									<img src="<?=$arItem['PICTURE_PRINT_BIG']['SRC']?>" alt="<?=$arItem['PICTURE_PRINT_BIG']['ALT']?>">
							</noscript>
						</span>
					</a>
					<header><?=$productTitle?></header>
					...
					<ul class="techdata">
						<?foreach($arItem['DISPLAY_PROPERTIES'] as $arProp):?>
							<li><span class="text"><?=(is_array($arProp['VALUE']) ? implode(' / ',$arProp['VALUE']) : $arProp['VALUE'])?></span></li>
						<?endforeach?>
					</ul>
					<div class="desc"><p><?=$arItem['PREVIEW_TEXT']?></p></div>
					<div class="prices-wrap">
						<span id="<?=$arItemIDs['PRICE_DIV']?>">
							...
							<? if(!$arItem['ON_REQUEST']): ?>
								<?if($arItem['MIN_PRICE']['DISCOUNT_DIFF'] > 0):?>
									<span class="text"><?=GetMessage('BITRONIC2_COOL_SLIDER_OLD_PRICE')?></span>
								...
								<?endif?>
								<span class="text"><?=GetMessage('BITRONIC2_COOL_SLIDER_NEW_PRICE')?> </span>
							<? endif ?>
							<? $frame->end() ?>
						</span>
						<span class="price" id="<?=$arItemIDs['PRICE']?>">
							...
							<? else: ?>
								<?=($arItem['bOffers']) ? GetMessage('BITRONIC2_COOL_SLIDER_FROM') : ''?>
				...
				<div class="track">
					<?foreach($arResult['ITEMS'] as $arItem):?>
						<div class="ctrl-item <?=($arItem['bFirst']) ? 'active' : ''?>">
							...
							<i class="dot" data-tooltip title="<?=$arItem['NAME']?>"></i>
							<div class="content" data-tooltip title="<?=$arItem['NAME']?>">
								<div class="img-wrap">
									<span data-picture data-alt="<?=$arItem['PICTURE_PRINT_SMALL']['ALT']?>">
										<span data-src="<?=$arItem['PICTURE_PRINT_SMALL']['SRC']?>"></span>
										<span data-src="" data-media="(max-width: 991px)"></span>
										
										<!-- Fallback content for non-JS browsers. Same img src as the initial, unqualified source element. -->
										<noscript>
												<img src="<?=$arItem['PICTURE_PRINT_SMALL']['SRC']?>" alt="<?=$arItem['PICTURE_PRINT_SMALL']['ALT']?>">
										</noscript>
									</span>
								</div>
