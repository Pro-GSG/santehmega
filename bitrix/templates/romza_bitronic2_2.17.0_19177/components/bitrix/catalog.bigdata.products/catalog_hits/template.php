...
<div id="<?=$injectId?>_items" class="catalog-hits<?=($arParams['SHOW_MINIFIED']?' hits-hidden':' hits-shown')?><?= $bHoverMode ? ' __expanded' : '' ?>">
	...
	?><div class="hits-item<?=($arItem['CAN_BUY']?'':' out-of-stock')?><?if($bHoverMode):?> hover_expand_item<?endif?>" id="<?=$arItemIDs['ID']?>">
		<div class="photo"><a href="<?=$arItem['DETAIL_PAGE_URL']?>"><img src="<?=$arItem['PICTURE_PRINT']['SRC']?>" alt="<?=$arItem['PICTURE_PRINT']['ALT']?>"></a></div>
		...
		<?else:?>
			...
			<span class="price" id="<?=$arItemIDs['PRICE']?>">
				<?=($arItem['bOffers']) ? GetMessage('BITRONIC2_ACCESORIES_FROM') : ''?>
...
<? endif ?>
		...
		<? endif ?>
			<div class="action-buttons" id="<?=$arItemIDs['BASKET_ACTIONS']?>">
				...
				<?endif?>
				<?if ($arParams['DISPLAY_COMPARE']/* && !$arItem['bOffers']*/):?>
					<button
						type="button"
