						...
						<div class="combo-target-content">
							...
							<? if (isset($arGroupFieldSets['BY_GROUPS'][$groupID])):?>
								<? foreach($arGroupFieldSets['BY_GROUPS'][$groupID] as $fieldCode => $arItems):
									...
									<div class="col-xs-12">
										<fieldset class="<?=( $arResult['FIELDSET'][$fieldCode]['class'] ? $arResult['FIELDSET'][$fieldCode]['class'] : 'row' )?><?=($groupID=='blocks'?' no-border':'')?>">
											<legend <?if($groupID == 'blocks'):?> class="collapsed"<?endif?> data-toggle="collapse" data-target="#<?=$fieldCode,$groupID?>"><?= $arResult['FIELDSET'][$fieldCode]['name'] ?></legend>
											<div class="collapse<?if($groupID != 'blocks'):?> in<?endif?>" id="<?=$fieldCode,$groupID?>">
											<?
